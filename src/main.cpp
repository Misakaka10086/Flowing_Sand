...
void loop()
{
    ...
    {
        if (codeStreams[x].isActive)
        { // <--- RENAMED (and all subsequent uses of codeStreams[x].MEMBER)
...
#ifdef DEBUG_SERIAL
                if (Serial && loopCounter % 20 == 0)
                    Serial.printf("Stream %d deactivated.\n", x);
#endif
            }
        }
        else
        {
            if (currentTimeMs - codeStreams[x].lastActivityTimeMs >= codeStreams[x].spawnCooldownMs)
            {
                if (random(1000) / 1000.0f < SPAWN_PROBABILITY)
                {
                    ...
                    if (codeStreams[x].hue < 0.0f)
                        codeStreams[x].hue += 1.0f;
                    if (codeStreams[x].hue > 1.0f)
                        codeStreams[x].hue -= 1.0f;
                    codeStreams[x].lastActivityTimeMs = currentTimeMs;
#ifdef DEBUG_SERIAL
                    if (Serial)
                        Serial.printf("Stream %d ACTIVATED: Y=%.1f, Spd=%.1f, Len=%d\n", x, codeStreams[x].currentY, codeStreams[x].speed, codeStreams[x].length);
#endif
                }
            }
        }
    ...
    {
        if (codeStreams[x].isActive)
        { // <--- RENAMED (and all subsequent uses)
            for (int l = 0; l < codeStreams[x].length; ++l)
            {
                ...
                if (charY >= 0 && charY < MATRIX_HEIGHT)
                {
                    ...
                    HsbColor charColor(codeStreams[x].hue, SATURATION, brightnessFactor);
