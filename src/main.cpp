        ...
        for (int i_pixel = 0; i_pixel < MATRIX_WIDTH; ++i_pixel)
        {
            ...
            for (int r_idx = 0; r_idx < MAX_RIPPLES; ++r_idx)
            {
                if (ripples[r_idx].isActive)
                {
                    ...
                    // 波纹消失条件
                    if (currentRadius > ripples[r_idx].maxRadius + RIPPLE_THICKNESS / 2.0f)
                    { //  /2 so it fades out at edge
                        ripples[r_idx].isActive = false;
#ifdef DEBUG_SERIAL
                        if (Serial && loopCount % 50 == 0)
                        { // Less frequent deactivation log
                    ...
                    if (distToRippleEdge < RIPPLE_THICKNESS / 2.0f)
                    {
                        ...
                        if (intensityFactor > maxIntensityForThisPixel)
                        {
                            ...
                            colorForThisPixel = HsbColor(ripples[r_idx].hue, 1.0f, maxIntensityForThisPixel);
                        }
                    }
                }
            }
...
#ifdef DEBUG_SERIAL
    if (loopCount % 100 == 0 && Serial)
    { // Print strip.Show status less frequently
