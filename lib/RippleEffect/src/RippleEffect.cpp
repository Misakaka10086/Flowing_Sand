...
const RippleEffect::Parameters RippleEffect::WaterDropPreset = {
    ...
    .baseBrightness = 0.8f,
    .sharpness =
        1.0f, // ADDED: A value of 1.0 gives the original soft cosine falloff
    .prePara = "WaterDrop"};
...
const RippleEffect::Parameters RippleEffect::EnergyPulsePreset = {
    ...
    .baseBrightness = 0.9f,
    .sharpness =
        2.5f, // ADDED: A higher value for a sharper, more defined pulse
    .prePara = "EnergyPulse"};
...
void RippleEffect::setParameters(const Parameters &params) {
  ...
  if (newTarget.maxRipples != _activeParams.maxRipples || _ripples == nullptr) {
    ...
    // _targetParams must be set before new Ripple allocation if maxRipples
    // comes from there. However, newTarget already has the correct maxRipples.
...
void RippleEffect::setParameters(const char *jsonParams) {
  ...
  if (doc["maxRipples"].is<uint8_t>())
    newParams.maxRipples = doc["maxRipples"].as<uint8_t>();
  if (doc["speed"].is<float>())
    newParams.speed = doc["speed"].as<float>();
  if (doc["thickness"].is<float>())
    newParams.thickness = doc["thickness"].as<float>();
  if (doc["spawnIntervalS"].is<float>())
    newParams.spawnIntervalS = doc["spawnIntervalS"].as<float>();
  if (doc["maxRadius"].is<float>())
    newParams.maxRadius = doc["maxRadius"].as<float>();
  if (doc["randomOrigin"].is<bool>())
    newParams.randomOrigin = doc["randomOrigin"].as<bool>();
  if (doc["saturation"].is<float>())
    newParams.saturation = doc["saturation"].as<float>();
  if (doc["baseBrightness"].is<float>())
    newParams.baseBrightness = doc["baseBrightness"].as<float>();
  if (doc["sharpness"].is<float>())
    newParams.sharpness = doc["sharpness"].as<float>();

  if (doc["prePara"].is<const char *>()) {
    const char *presetStr = doc["prePara"].as<const char *>();
    if (presetStr) {
      if (strcmp(presetStr, WaterDropPreset.prePara) == 0)
        newParams.prePara = WaterDropPreset.prePara;
      else if (strcmp(presetStr, EnergyPulsePreset.prePara) == 0)
        newParams.prePara = EnergyPulsePreset.prePara;
    }
  }
...
void RippleEffect::setPreset(const char *presetName) {
  ...
  if (strcmp(presetName, "next") == 0) {
    const char *currentEffectivePreset =
        _effectInTransition ? _targetParams.prePara : _activeParams.prePara;
    if (currentEffectivePreset == nullptr)
      currentEffectivePreset = WaterDropPreset.prePara;
...
int RippleEffect::mapCoordinatesToIndex(int x, int y) {
  ...
  int base_index;
  if (module_row == 1 && module_col == 1)
    base_index = 0;
  else if (module_row == 1 && module_col == 0)
    base_index = leds_per_module * 1;
  else if (module_row == 0 && module_col == 1)
    base_index = leds_per_module * 2;
  else
    base_index = leds_per_module * 3;
  int local_x = x % module_width;
  int local_y = y % module_height;
  int local_offset = (module_height - 1 - local_y) * module_width +
                     (module_width - 1 - local_x);
...
void RippleEffect::Update() {
  if (_effectInTransition) {
    ...
    unsigned long elapsedTime =
        currentTimeMs_lerp - _effectTransitionStartTimeMs;
    float t = static_cast<float>(elapsedTime) / _effectTransitionDurationMs;
    t = (t < 0.0f) ? 0.0f : (t > 1.0f) ? 1.0f : t; // Clamp t

    // Interpolate parameters (maxRipples changes instantly and is handled in
    // setParameters)
    _activeParams.speed = lerp(_oldParams.speed, _targetParams.speed, t);
    _activeParams.thickness =
        lerp(_oldParams.thickness, _targetParams.thickness, t);
    _activeParams.spawnIntervalS =
        lerp(_oldParams.spawnIntervalS, _targetParams.spawnIntervalS, t);
    _activeParams.maxRadius =
        lerp(_oldParams.maxRadius, _targetParams.maxRadius, t);
    _activeParams.randomOrigin =
        (t < 0.5f) ? _oldParams.randomOrigin
                   : _targetParams.randomOrigin; // bool, switch halfway
    _activeParams.saturation =
        lerp(_oldParams.saturation, _targetParams.saturation, t);
    _activeParams.baseBrightness =
        lerp(_oldParams.baseBrightness, _targetParams.baseBrightness, t);
    _activeParams.sharpness =
        lerp(_oldParams.sharpness, _targetParams.sharpness, t);
  ...
  if (_strip == nullptr || _ripples == nullptr || _matrixWidth == 0)
    return;

  unsigned long currentTimeMs = millis();

  // Auto-creation logic is unchanged
  if (currentTimeMs - _lastAutoRippleTimeMs >=
      (unsigned long)(_activeParams.spawnIntervalS * 1000.0f)) {
    ...
    for (int x = 0; x < _matrixWidth; ++x) {
      ...
      for (int r_idx = 0; r_idx < _activeParams.maxRipples; ++r_idx) {
        if (_ripples[r_idx].isActive) {
          float elapsedTimeS =
              (float)(currentTimeMs - _ripples[r_idx].startTimeMs) / 1000.0f;
          float currentRadius = elapsedTimeS * _activeParams.speed -
                                _activeParams.thickness / 2.0f;

          if (currentRadius >
              _activeParams.maxRadius + _activeParams.thickness / 2.0f) {
            _ripples[r_idx].isActive = false;
            continue;
          }

          float distToOrigin =
              sqrt(pow(pixelCenterX - _ripples[r_idx].originX, 2) +
          ...
          if (distToRippleEdge < _activeParams.thickness / 2.0f) {
            ...
            // 1. Calculate the base intensity using the cosine curve (from 0.0
            // to 1.0)
            float baseIntensity =
                cos((distToRippleEdge / (_activeParams.thickness / 2.0f)) *
                    (PI / 2.0f));
            ...
            if (intensityFactor > maxIntensityForThisPixel) {
              ...
              colorForThisPixel = HsbColor(
                  _ripples[r_idx].hue, _activeParams.saturation,
                  maxIntensityForThisPixel * _activeParams.baseBrightness);
            }
          }
        }
      }
