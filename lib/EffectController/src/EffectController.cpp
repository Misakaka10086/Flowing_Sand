...
void EffectController::Update() {
  ...
  case EffectType::SCROLLING_TEXT:
    scrollingTextEffect.Update();
    break;
  case EffectType::LAVA_LAMP:
    lavaLampEffect.Update();
    break;
...
void EffectController::processCommand(const char *jsonCommand) {
  ...
  if (effectName) {
    if (strcmp(effectName, "gravity_balls") == 0)
      _currentEffect = EffectType::GRAVITY_BALLS;
    else if (strcmp(effectName, "zen_lights") == 0)
      _currentEffect = EffectType::ZEN_LIGHTS;
    else if (strcmp(effectName, "code_rain") == 0)
      _currentEffect = EffectType::CODE_RAIN;
    else if (strcmp(effectName, "ripple") == 0)
      _currentEffect = EffectType::RIPPLE;
    else if (strcmp(effectName, "scrolling_text") == 0)
      _currentEffect = EffectType::SCROLLING_TEXT;
    else if (strcmp(effectName, "lava_lamp") == 0)
      _currentEffect = EffectType::LAVA_LAMP;
    else if (strcmp(effectName, "animation") == 0)
      _currentEffect = EffectType::ANIMATION_EFFECT;
